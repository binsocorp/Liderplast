...
export function OrderDetailClient({
    ...
    let initialColor = 'Celeste';
    if (cascoObj && cascoObj.description) {
        const colorMatch = cascoObj.description.match(/\(Color:\s*([^)]+)\)/i);
        if (colorMatch) initialColor = colorMatch[1];
    }
        ...
        <>
            <div className="max-w-6xl mx-auto space-y-6 pt-6 pb-20 text-sm bg-white overflow-hidden">
                ...
                <div className="flex items-center gap-4 border-b border-gray-100 pb-6 mb-2">
                    <div className="w-14 h-14 bg-primary-50 text-primary-700 rounded-2xl flex items-center justify-center font-black text-2xl border border-primary-100 shadow-sm rotate-3">Ed</div>
                    <div className="flex-1">
                        <h1 className="text-3xl font-black tracking-tight text-gray-900 leading-none">Editar Pedido</h1>
                        <p className="text-gray-400 font-medium mt-1">Cotización #{order.order_number}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="space-y-1.5 col-span-2">
                        <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1">Cliente</label>
                        <Input value={clientName} onChange={e => setClientName(e.target.value)} required className="h-11 w-full rounded-xl font-bold border border-gray-200 focus:ring-4 focus:ring-primary-500/10" placeholder="Nombre completo" />
                    </div>
                    <div className="space-y-1.5 col-span-1">
                        <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1">DNI</label>
                        <Input value={document} onChange={e => setDocument(e.target.value)} className="h-11 rounded-xl border border-gray-200" />
                    </div>
                    <div className="space-y-1.5 col-span-1">
                        <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1">Teléfono</label>
                        <Input value={phone} onChange={e => setPhone(e.target.value)} className="h-11 rounded-xl border border-gray-200" />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                    <div className="space-y-1.5 col-span-3">
                        ...
                        <Input value={address} onChange={e => setAddress(e.target.value)} required className="h-11 rounded-xl bg-white border border-gray-200" />
                    </div>
                    <div className="space-y-1.5 col-span-1">
                        <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1">Distancia (Km)</label>
                        <Input type="number" value={distance} onChange={e => setDistance(e.target.value)} className="h-11 rounded-xl bg-white text-center font-bold border border-gray-200" />
                    </div>
                </div>

                {/* SECTION: PRODUCT TITLE */}
                <div className="bg-gray-950 px-6 py-3 rounded-2xl flex items-center gap-2 shadow-lg mt-8">
                ...
                <div className="overflow-hidden rounded-2xl border border-gray-100 shadow-sm mt-4">
                    ...
                    <div className="divide-y divide-gray-200">
                        <div className="grid grid-cols-[1fr_300px_150px] items-center">
                            <div className="py-2.5 px-3 border-r border-gray-200 font-bold text-gray-700 text-xs uppercase tracking-tight ml-1">Casco</div>
                            <div className="py-1.5 px-3 border-r border-gray-200 flex justify-center">
                                <Select value={cascoId} onChange={e => setCascoId(e.target.value)} options={cascos.map((c: any) => ({ value: c.id, label: c.name }))} uiSize="sm" className="h-8 w-full border border-gray-200" />
                            </div>
                            <div className="py-2.5 px-4 text-right font-black tracking-tighter text-gray-900">{formatCurrency(cascoId ? getPrice(cascoId) : 0)}</div>
                        </div>

                        <div className="grid grid-cols-[1fr_300px_150px] items-center">
                            <div className="py-2.5 px-3 border-r border-gray-200 font-bold text-gray-700 text-xs uppercase tracking-tight ml-1 flex items-center justify-between">
                                <span>Color</span>
                        ...
                        <div className="grid grid-cols-[1fr_300px_150px] items-center hover:bg-gray-50/50 transition-colors">
                            <div className="py-2.5 px-4 border-r border-gray-100 font-bold text-gray-700 select-none text-xs uppercase tracking-tight">Losetas L</div>
                            <div className="py-1.5 px-4 border-r border-gray-100 flex justify-center">
                                <Input type="number" min="0" value={losetasL} onChange={e => setLosetasL(e.target.value)} uiSize="sm" className="h-8 w-20 text-center font-bold border border-gray-200 shadow-sm" />
                            </div>
                            <div className="py-2.5 px-4 text-right font-black tracking-tighter text-gray-900">{formatCurrency(Number(losetasL || 0) * getPrice(getItemId('Loseta Atérmica L')))}</div>
                        </div>

                        <div className="grid grid-cols-[1fr_300px_150px] items-center hover:bg-gray-50/50 transition-colors">
                            <div className="py-2.5 px-4 border-r border-gray-100 font-bold text-gray-700 select-none text-xs uppercase tracking-tight">Losetas R</div>
                            <div className="py-1.5 px-4 border-r border-gray-100 flex justify-center">
                                <Input type="number" min="0" value={losetasR} onChange={e => setLosetasR(e.target.value)} uiSize="sm" className="h-8 w-20 text-center font-bold border border-gray-200 shadow-sm" />
                            </div>
                            <div className="py-2.5 px-4 text-right font-black tracking-tighter text-gray-900">{formatCurrency(Number(losetasR || 0) * getPrice(getItemId('Loseta Atérmica R')))}</div>
                        </div>

                        <div className="grid grid-cols-[1fr_300px_150px] items-center hover:bg-gray-50/50 transition-colors">
                            <div className="py-2.5 px-4 border-r border-gray-100 font-bold text-gray-700 select-none text-xs uppercase tracking-tight">Pastina (Kg)</div>
                            <div className="py-1.5 px-4 border-r border-gray-100 flex justify-center">
                                <Input type="number" min="0" value={pastina} onChange={e => setPastina(e.target.value)} uiSize="sm" className="h-8 w-20 text-center font-bold border border-gray-200 shadow-sm" />
                            </div>
                            <div className="py-2.5 px-4 text-right font-black tracking-tighter text-gray-900">{formatCurrency(Number(pastina || 0) * getPrice(getItemId('Pastina (Kg)')))}</div>
                        </div>
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 flex flex-col md:flex-row gap-6 mt-8">
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 mt-8">
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1 mb-4 block">Asignar Flete / Viaje</label>
                    <div className="flex gap-4 items-center">
                        <Select
                            ...
                            className="h-12 rounded-xl flex-1 bg-white border border-gray-200 focus:ring-4 focus:ring-primary-500/10 font-bold"
                        />
                        {tripId && (
                            <Button
                                size="lg"
                ...
                {/* GRAND TOTAL */}
                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6 items-center bg-gray-900 p-6 rounded-3xl shadow-2xl border border-gray-700">
