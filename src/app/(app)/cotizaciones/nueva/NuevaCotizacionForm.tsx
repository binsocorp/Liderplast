...
// Default: hoy + 30 días en formato YYYY-MM-DD
...
export function NuevaCotizacionForm({
    ...
    // Auto-fill flete e instalación al cambiar provincia/canal
    useEffect(() => {
    ...
    // Auto-fill lista de precios para REVENDEDOR
    useEffect(() => {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            // Armar array de ítems de producto
            const items: any[] = [];
            const pushItem = (id: string, desc: string, qty: number, price: number) => {
                if (id && qty > 0) {
                    items.push({ catalog_item_id: id, description: desc, quantity: qty, unit_price_net: price, type: 'PRODUCTO' });
                }
            };
            ...
            if (colorExtra > 0) {
                ...
                if (colorItemId) pushItem(colorItemId, `Adicional por Color (${color})`, 1, colorExtra);
            }
            ...
            if (result.error || !result.data) {
                setError(result.error || 'Error al crear la cotización');
                setLoading(false);
                return;
            }
    ...
    return (
        <>
            <PageHeader title="Nueva Cotización" backHref="/cotizaciones" />
            ...
            <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-6 p-6 pb-20 text-sm bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div className="flex items-center gap-4 border-b border-gray-100 pb-6 mb-2">
                    <div>
                        <h1 className="text-3xl font-black tracking-tight text-gray-900 leading-none">Nueva Cotización</h1>
                        <p className="text-gray-400 font-medium mt-1">Generá una cotización formal para enviar al cliente</p>
                    </div>
                </div>

                {/* ROW 1: Canal, Vendedor, Revendedor */}
                ...
                {/* ROW 2: Provincia, Localidad, Validez */}
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="space-y-1.5 col-span-2">
                        <label className="text-[11px] font-black text-amber-600 uppercase tracking-widest px-1">Válida hasta</label>
                        <Input
                            type="date"
                            value={expiresAt}
                            onChange={e => setExpiresAt(e.target.value)}
                            className="h-11 rounded-xl border border-amber-200 bg-amber-50/20 focus:ring-4 focus:ring-amber-500/10 transition-all font-bold text-amber-700"
                        />
                    </div>
                </div>

                {/* ROW 3: Datos de contacto */}
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="space-y-1.5 col-span-2">
                ...
                {/* NOTES */}
                <div className="space-y-1.5">
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest px-1">Notas internas</label>
                    <textarea
                        value={notes}
                        onChange={e => setNotes(e.target.value)}
                        rows={2}
                        placeholder="Observaciones, condiciones especiales..."
                        className="w-full rounded-xl border border-gray-200 px-4 py-3 text-sm focus:outline-none focus:ring-4 focus:ring-primary-500/10 resize-none"
                    />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center bg-gray-900 p-6 rounded-3xl shadow-2xl border border-gray-700">
                    <div className="space-y-1">
                        <h3 className="text-amber-400 font-black text-2xl tracking-tighter uppercase">{formatCurrency(totalAPagar)}</h3>
                        <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Total Cotizado</p>
                        {expiresAt && (
                            <p className="text-gray-500 text-[10px] font-bold uppercase tracking-widest">
                                Válida hasta: {new Date(expiresAt + 'T00:00:00').toLocaleDateString('es-AR')}
                            </p>
                        )}
                    </div>
                    <div className="flex justify-end gap-3">
                        ...
                        <Button
                            type="submit"
                            disabled={loading}
                            className="h-14 px-10 rounded-2xl bg-amber-600 hover:bg-amber-500 text-white font-black uppercase text-xs tracking-widest transition-all shadow-xl shadow-amber-900/40 active:scale-95"
                        >
